...
def fidelity(Utarget, Uexp):
    ...
    if np.shape(Utarget) != np.shape(Uexp):
        ...
        return np.clip(np.abs(np.trace(Utarget.T.conj() @ Uexp) / dim), 0, 1)


def fidelity_real(Utarget, Uexp):
    """Returns only the real part of the trace of
    Utarget' * Uexp.
    """
    if np.shape(Utarget) != np.shape(Uexp):
        raise('Inputs are not the same shape')
    else:
        dim = np.size(Utarget, 0)
        return np.real(np.trace(Utarget.T.conj() @ Uexp) / dim)
